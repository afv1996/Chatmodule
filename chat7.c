...
static ssize_t chat_cdev_read(struct file *filp, char __user *buf, size_t len,loff_t * off);
static ssize_t chat_cdev_write(struct file *filp, const char *buf, size_t len, loff_t * off);
static int chat_cdev_release(struct inode *inode, struct file *file);
...
static struct file_operations chat_ioctl_fops =
{
	...
	.open =	chat_open, 
	.read = chat_cdev_read,
	.write = chat_cdev_write,
	.release = chat_cdev_release,
	.unlocked_ioctl = chat_ioctl
};
static int chat_cdev_open(struct inode *inode, struct file *file)
{
        printk(KERN_INFO "Device File Opened...!!!\n");
        return 0;
}
...
static ssize_t chat_cdev_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
